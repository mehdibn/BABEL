...
import tn.lipsic.babel.measurements.exporter.TextMeasurementsExporter;
import tn.lipsic.babel.workloads.CoreWorkload;
...
/**
 * Main class for executing babel.
 */
public final class Client {
     ...
     */
    public static final String INJECTOR_ID = "id";
    /**
     * The exporter class to be used. The default is
     * tn.lipsic.babel.measurements.exporter.TextMeasurementsExporter.
     */
    public static final String EXPORTER_PROPERTY = "exporter";
    /**
     * If set to the path of a file, babel will write all output to this file
     * instead of STDOUT.
     */
    public static final String EXPORT_FILE_PROPERTY = "exportfile";
    /**
     * The number of babel client threads to run.
     */
    ...
    public static void usageMessage() {
        System.out.println("Usage: java tn.lipsic.babel.Client [options]");
        ...
        System.out.println("  -producer dbname: specify the name of the GenericProducer to use (default: tn.lipsic.integration.producers.BasicDB) - \n" +
        ...
        System.out.println("  -id injectorID:  use an Id by Injector (default value : default)");

        System.out.println("");
        //System.out.println("Important properties:");
        //System.out.println("  " + WORKLOAD_PROPERTY + ": the name of the workload class to use (e.g. " +
        //        "tn.lipsic.babel.workloads.CoreWorkload)");
        System.out.println("");
    ...
    public static void main(String[] args) {
        ...
        String dbname = props.getProperty(DB_PROPERTY, "tn.lipsic.integration.producers.BasicDB");
    ...
    private static Tracer getTracer(Properties props, Workload workload) {
        return new Tracer.Builder("babel " + workload.getClass().getSimpleName())
    ...
    private static Workload getWorkload(Properties props) {
        ...
        try {
            ...
            System.err.println("babel Client " + projectProp.getProperty("version"));
        } catch (IOException e) {
        ...
        if (args.length != 0) {
            while (args[argindex].startsWith("-")) {
                if (args[argindex].compareTo("-threads") == 0) {
                    ...
                    if (argindex >= args.length) {
                        usageMessage();
                        System.out.println("Missing argument value for -id.");
                        System.exit(0);
                    }
                    props.setProperty(INJECTOR_ID, args[argindex]);
                    argindex++;
